...
function getProductFromLocalStorage(productId) {
  const data = JSON.parse(localStorage.getItem("selectedCategoryProducts")) || {};
...
async function getRelatedproducts() {
  ...
  try {
    const response = await fetch(`https://dummyjson.com/products/category/${singleProduct.category}?limit=10`);
    if (!response.ok) throw new Error("API Response is not ok");
    ...
    const filterItems = relatedItems.filter((item) => item.id !== currentProductId);
...
function renderProductDetails(product) {
  ...
  eachProduct.className = "singleProduct flex gap-8 mb-8";
  eachProduct.innerHTML = `
    <div class="leftDiv w-1/2">
    <div class="flex justify-center rounded shadow bg-neutral-50 p-4">
      <img src="${product.images?.[0]}" alt="${product.title}" class="rounded" />
      </div>
    </div>
    <div class="rightDiv w-1/2">
     <div class="flex gap-6">
        <span class="text-blue-600 dark:text-sky-400 font-bold">${product.brand}</span>
      </div>
      <div class="flex justify-between items-start mb-2">
        <h2 class="text-2xl/7 font-bold text-gray-900 sm:truncate sm:text-3xl">${product.title}</h2>
        <span class="inline-flex items-center rounded-md bg-green-50 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-green-600/20 ring-inset">${product.availabilityStatus || "In Stock"}</span>
      </div>
      
        <p class="mb-4 text-yellow-500 text-lg">${getStarRating(product.rating)} <span class="text-gray-700 text-sm">(${product.rating})</span></p>
     
      <div class="mb-6">
        <div class="text-blue-600 dark:text-sky-400 font-bold text-lg mb-2">$${product.price}</div>
        <button id="${product.id}"
          data-title="${product.title}" 
          data-price="${product.price}"
      ...
      <p class="mb-4 blink">⛟ ${product.shippingInformation || "Ships in 3-5 days"}</p>
      <p class="text-gray-700 mb-4">${product.description}</p>
      <div class="mb-4">
       <h5 class="font-semibold mb-2">Warranty & Return Policy</h5>
       <div class="flex gap-8 items-start">
       <p><span class='text-lg'>✪</span> ${product.warrantyInformation || 'No Warranty'}</p>
      ...
      <div class="">
        ...
        <ul class="list-disc-none">
          ${product.reviews ? product.reviews.map(review => `<li class="mb-4 text-sm">
            ...
            <span class="text-neutral-400 text-xs">${new Date(review.date).toLocaleDateString()}</span>
            </div>
            
            </li>`).join('') : '<li>No recent reviews available</li>'}
        </ul>
        </div>
